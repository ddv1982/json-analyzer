...
export function useJsonAnalyzerState() {
  ...
  const [activeTab, setActiveTab] = useState<ResultTab>('statistics')
  ...
  async function runDebouncedValidation(jsonString: string, requestId: number) {
    ...
    try {
      const result = await validateJson({ json_string: jsonString })
  ...
  async function handleFormat() {
    ...
    try {
      const result = await formatJson({ json_string: jsonInput })
  ...
  async function handleAnalyze() {
    ...
    try {
      validationResult = await validateJson({ json_string: jsonInput })
    ...
    try {
      const analysisResult = await analyzeJson({
