#!/usr/bin/env python3
"""Build a minimal signed JSON Analyzer APT repository from .deb artifacts.

The generated tree is intentionally static-hosting friendly:

  pool/main/j/json-analyzer/*.deb
  dists/<suite>/Release
...
DEFAULT_PACKAGE = "json-analyzer"
DEFAULT_COMPONENT_ID = "com.jsonanalyzer.desktop"
DEFAULT_ORIGIN = "json-analyzer-stable-main"
DEFAULT_LABEL = "JSON Analyzer"
DEFAULT_DESCRIPTION = "JSON Analyzer APT repository"
DEFAULT_REPOSITORY_URL = "https://ddv1982.github.io/json-analyzer/apt/"
DEFAULT_SETUP_PACKAGE_NAME = "json-analyzer-repository-setup"
DEFAULT_SETUP_PACKAGE_VERSION = "1.0"
DEFAULT_SETUP_MAINTAINER = "JSON Analyzer <noreply@jsonanalyzer.invalid>"
DEFAULT_SETUP_KEYRING_PATH = "/usr/share/keyrings/json-analyzer-archive-keyring.pgp"
DEFAULT_SETUP_SOURCES_PATH = "/etc/apt/sources.list.d/json-analyzer.sources"
...
def build_setup_package(args: argparse.Namespace, architectures: list[str]) -> None:
    ...
    control = (
        ...
        "Description: JSON Analyzer APT repository setup package\n"
        " Installs the JSON Analyzer archive keyring and Deb822 source configuration.\n"
    ).encode("utf-8")
    control_files = {
...
def build_repository(args: argparse.Namespace) -> None:
    ...
    for deb in args.deb:
        ...
        pool_rel = pathlib.PurePosixPath("pool/main/j/json-analyzer") / normalized_pool_filename(fields)
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Build a minimal signed JSON Analyzer APT repository.")
    ...
    parser.add_argument(
        "--setup-package-out",
        help="optional path for json-analyzer-repository-setup_<version>_all.deb output",
    )
    parser.add_argument(
        "--setup-public-key",
